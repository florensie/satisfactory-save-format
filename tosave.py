#!/usr/bin/env python3
"""
Converts from a Python dict back to a Satisfactory save game (.sav)
"""
...
def to_save(save_dict):
    output = open("output.sav", 'wb')
    ...
    # Header
    write_int(save_dict['save_header_type'])
    write_int(save_dict['save_version'])
    write_int(save_dict['build_version'])
    write_length_prefixed_string(save_dict['map_name'])
    write_length_prefixed_string(save_dict['map_options'])
    write_length_prefixed_string(save_dict['session_name'])
    write_int(save_dict['play_duration_seconds'])
    write_long(save_dict['save_date_time'])
    write_byte(save_dict['session_visibility'])

    write_int(len(save_dict['objects']))
